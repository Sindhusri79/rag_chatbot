...
This project implements a **PDF-based Retrieval-Augmented Generation (RAG) Chatbot** using Python. It allows users to upload a PDF document, automatically extracts its text, divides the content into smaller chunks, generates embeddings, stores them in a vector database, and answers user questions by retrieving the most relevant information from the PDF.

Instead of searching the entire document every time, the chatbot retrieves only the most relevant sections and provides accurate, context-aware responses using a Large Language Model (LLM).
